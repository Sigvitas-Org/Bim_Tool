...
<div class="modal-body bg-grey">
    ...
    <div class="row">
        <div class="col-md-6">
            <x-cards.data :title="__('app.date').' - '.$attendanceDate->format(company()->date_format)">
                <div class="punch-status">
                    <div class="border rounded p-3 mb-3 bg-light">
                        <h6 class="f-13">@lang('modules.attendance.clock_in')</h6>
                        <p class="mb-0">{{ $startTime->format(company()->time_format) }}</p>
                    </div>
                    ...
                    <div class="border rounded p-3 bg-light">
                        ...
                        <p class="mb-0">{{ $endTime != '' ? $endTime->format(company()->time_format) : '' }}
                            ...
                            <ul class="res-activity-list col-md-9">
                                <li>
                                    ...
                                    <p class="res-activity-time">
                                        ...
                                        {{ $item->clock_in_time->timezone(company()->timezone)->format(company()->date_format . ' ' . company()->time_format) }}
                                ...
                                <li>
                                    ...
                                    <p class="res-activity-time">
                                        ...
                                        @if (!is_null($item->clock_out_time))
                                            {{ $item->clock_out_time->timezone(company()->timezone)->format(company()->date_format . ' ' . company()->time_format) }}
                                        @else
                                            @lang('modules.attendance.notClockOut')
                                        @endif
                                    </p>
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0 mr-2"
                                        ...
                                        @if ($addAttendancePermission == 'all' && $maxClockIn)
                                            <a class="dropdown-item d-block text-dark-grey f-13 py-1 px-3"
                                                href="javascript:;" onclick="addAttendance({{ $item->user_id }})"
                                                data-attendance-id="{{ $item->user_id }}">@lang('app.add')</a>
                                        @endif
                                        @if ($editAttendancePermission == 'all'
                                            || ($editAttendancePermission == 'added' && $item->added_by == user()->id)
                                            || ($editAttendancePermission == 'owned' && $attendance->user->id == user()->id)
