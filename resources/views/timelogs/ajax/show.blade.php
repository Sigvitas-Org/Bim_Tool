            ...
            <div class="card bg-white border-0 b-shadow-4">
                ...
                <div class="card-body">
                    <x-cards.data-row :label="__('modules.timeLogs.startTime')"
                        :value="$timeLog->start_time->timezone(company()->timezone)->format(company()->date_format . ' ' . company()->time_format)" />

                    @if (!is_null($timeLog->end_time))
                        <x-cards.data-row :label="__('modules.timeLogs.endTime')"
                            :value="$timeLog->end_time->timezone(company()->timezone)->format(company()->date_format . ' ' . company()->time_format)" />
                ...
                <div class="card-body">
                    <ul class="list-group">
                        <li class="list-group-item d-flex justify-content-between align-items-center f-12 text-dark-grey">
                            ...
                            {{ $timeLog->start_time->timezone(company()->timezone)->format(company()->date_format .' '.company()->time_format) }}
                        </li>
                        <li class="list-group-item d-flex justify-content-between align-items-center f-12 text-dark-grey">
                        ...
                        @foreach ($timeLog->breaks as $item)
                            <li class="list-group-item d-flex justify-content-between align-items-center f-12 text-dark-grey">
                                @if (!is_null($item->end_time))
                                    ...
                                    @endphp
                                    <span><i class="fa fa-mug-hot"></i> @lang('modules.timeLogs.break') ({{ $timeLogHours }})</span>
                                    <span>
                        ...
                        @if (!is_null($timeLog->end_time))
                            <li class="list-group-item d-flex justify-content-between align-items-center f-12 text-dark-grey">
                                ...
                                {{ $timeLog->end_time->timezone(company()->timezone)->format(company()->date_format . ' ' . company()->time_format) }}
                            </li>
                        @endif
                    </ul>
                </div>
