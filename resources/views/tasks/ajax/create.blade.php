        ...
        <x-form id="save-task-data-form">
            <div class="add-client bg-white rounded">
                ...
                <div class="row p-20">
                    ...
                    <div class="col-md-5 col-lg-3">
                        <x-forms.datepicker fieldId="task_start_date" fieldRequired="true"
                            ...
                            :fieldValue="(($task) ? $task->start_date->format(company()->date_format) : \Carbon\Carbon::now(company()->timezone)->format(company()->date_format))"
                    ...
                    <div class="col-md-5 col-lg-3 dueDateBox"  @if($task && is_null($task->due_date)) style="display: none" @endif>
                        <x-forms.datepicker fieldId="due_date" fieldRequired="true" :fieldLabel="__('app.dueDate')"
                            ...
                            :fieldValue="(($task && $task->due_date) ? $task->due_date->format(company()->date_format) : \Carbon\Carbon::now(company()->timezone)->format(company()->date_format))"/>
                    </div>
                                ...
                                <div class="col-lg-3 col-md-6">
                                    <x-forms.select fieldId="board_column_id" :fieldLabel="__('app.status')"
                                        ...
                                        @foreach ($taskboardColumns as $item)
                                            <option @if ($columnId == $item->id || ( !is_null($task) && $task->board_column_id == $item->id)) selected @elseif (company()->default_task_status == $item->id) selected @endif value="{{ $item->id }}">
                            ...
                            <x-forms.select fieldId="dependent_task_id" :fieldLabel="__('modules.tasks.dependentTask')"
                                ...
                                @foreach ($allTasks as $item)
                                    <option value="{{ $item->id }}">{{ $item->heading }} (@lang('app.dueDate'):
                                        @if(!is_null($item->due_date)) {{ $item->due_date->format(company()->date_format) }} @else - @endif )
                                    </option>
                                @endforeach
                            </x-forms.select>
                        </div>
                ...
                <x-form-actions>
                    ...
                    <x-forms.button-secondary class="mr-3 d-none d-md-block" id="save-more-task-form" icon="check-double">@lang('app.saveAddMore')
