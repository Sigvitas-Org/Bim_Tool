...
<div class="card border-0 invoice">
                    ...
                    <td align="right">
                        <table class="inv-num-date text-dark f-13 mt-3">
                            ...
                            <tr>
                                ...
                                <td class="border-left-0">{{ $invoice->issue_date->format(company()->date_format) }}
                                </td>
                            </tr>
                            ...
                            @if (empty($invoice->order_id) && $invoice->status === 'unpaid' && $invoice->due_date->year > 1)
                                <tr>
                                    ...
                                    <td class="border-left-0">{{ $invoice->due_date->format(company()->date_format) }}
                                    </td>
                                </tr>
                            @endif
                        </table>
                    ...
                    <td colspan="2">
                        <table class="inv-detail f-14 table-responsive-sm" width="100%">
                            <tr class="i-d-heading bg-light-grey text-dark-grey font-weight-bold">
                                ...
                                <td class="border-left-0" align="right" width="{{ $invoiceSetting->hsn_sac_code_show ? '17%' : '20%' }}">
    ...
    <div class="card-footer bg-white border-0 d-flex justify-content-start py-0 py-lg-4 py-md-4 mb-4 mb-lg-3 mb-md-3 ">
        ...
        <div class="d-flex">
            <div class="inv-action mr-3 mr-lg-3 mr-md-3 dropup">
                ...
                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton" tabindex="0">
                    ...
                    @if ($invoice->status == 'paid' && !in_array('client', user_roles()) && $invoice->credit_note == 0)
                        <a class="dropdown-item invoice-upload" href="javascript:;" data-toggle="tooltip"  data-invoice-id="{{ $invoice->id }}">
                    ...
                    @if (!in_array('client', user_roles()) && in_array('payments', $user->modules) && $invoice->credit_note == 0 && $invoice->status != 'draft' && $invoice->status != 'paid' && $invoice->status != 'canceled' && $invoice->send_status)
                    ...
                    @if ($invoice->credit_note == 0 && $invoice->status != 'draft' && $invoice->status != 'canceled' && $invoice->status != 'unpaid' && !in_array('client', user_roles()))
                        @if ($invoice->amountPaid() > 0)
                            @if ($invoice->status == 'paid')
                                <a class="dropdown-item" href="{{ route('creditnotes.create') . '?invoice=' . $invoice->id }}"><i class="fa fa-plus mr-2"></i>@lang('modules.credit-notes.addCreditNote')</a>
                            @else
                                <a class="dropdown-item unpaidAndPartialPaidCreditNote" data-toggle="tooltip"  data-invoice-id="{{ $invoice->id }}" href="javascript:;"><i class="fa fa-plus mr-2"></i>@lang('modules.credit-notes.addCreditNote')</a>
                            @endif
                        @endif
                    @endif
                    ...
                    @if ($addInvoicesPermission == 'all' || $addInvoicesPermission == 'added')
                        <a href="{{ route('invoices.create') . '?invoice=' . $invoice->id }}" class="dropdown-item"><i class="fa fa-copy mr-2"></i> @lang('app.create') @lang('app.duplicate')</a>
                    @endif
            ...
            {{-- PAYMENT GATEWAY --}}
            @if (in_array('client', user_roles()) && $invoice->total > 0 && (in_array($invoice->status,['unpaid','partial'])) && ($credentials->show_pay || $methods->count() > 0))
                ...
                <div class="inv-action mr-3 mr-lg-3 mr-md-3 dropup">
                    ...
                    <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton" tabindex="0">
                        ...
                        @if ($credentials->flutterwave_status == 'active')
                            <li>
                                <a class="dropdown-item f-14 text-dark" href="javascript:void(0);" data-invoice-id="{{ $invoice->id }}"  id="flutterwaveModal">
