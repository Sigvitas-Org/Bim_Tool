                    ...
                    <td align="right">
                        <table class="inv-num-date text-dark f-13 mt-3">
                            ...
                            <tr>
                                ...
                                <td class="border-left-0">{{ $invoice->valid_till->format(company()->date_format) }}
                                </td>
                            </tr>
                        </table>
                    </td>
            ...
            <div class="inv-action dropup">
                ...
                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton" tabindex="0">
                    ...
                    @if ($invoice->status == 'waiting' || $invoice->status == 'draft')
                        ...
                        @endif
                        <li>
                        <a class="dropdown-item btn-copy" data-clipboard-text="{{ route('front.estimate.show', $invoice->hash) }}">
                        ...
                        @if ($invoice->status != 'canceled' && $invoice->status != 'accepted' && !in_array('client', user_roles()))
                            <li>
                                <a href="javascript:;" data-toggle="tooltip"  data-estimate-id="{{ $invoice->id }}" class="dropdown-item sendButton"><i class="fa fa-paper-plane mr-2"></i> @lang('app.send')</a>
                            </li>
                        @endif
                        <li>
                    ...
                    @if ($addEstimatePermission == 'all' || $addEstimatePermission == 'added')
                        <li>
                            <a href="{{ route('estimates.create') . '?estimate=' . $invoice->id }}" class="dropdown-item"><i class="fa fa-copy mr-2"></i> @lang('app.create') @lang('app.duplicate')</a>
                        </li>
                    @endif
                    @if ($firstEstimate->id == $invoice->id)
                        @if (
                            ...
                            || ($deleteEstimatePermission == 'both' && ($invoice->client_id == user()->id || $invoice->added_by == user()->id))
                        )
                        <li>
