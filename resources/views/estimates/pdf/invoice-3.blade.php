...
<div id="container">
    ...
    <section id="sums">
        ...
        <table cellpadding="0" cellspacing="0">
            ...
            <tr class="amount-total">
                ...
                <td>{{ currency_formatter($estimate->total, '') }} {!! htmlentities($estimate->currency->currency_code)  !!}</td>
            </tr>
        </table>
    ...
    <section id="terms">
        <div class="notes">
            <div>
                <span>@lang('modules.estimates.validTill'):</span> <span>{{ $estimate->valid_till ? $estimate->valid_till->format(company()->date_format) : '--' }}</span>
            </div>
            @if($estimate->status == 'unpaid')
            <div>
                <span>@lang('app.dueDate'):</span> <span>{{ $estimate->due_date->format(company()->date_format) }}</span>
            </div>
            @endif
            <div>
