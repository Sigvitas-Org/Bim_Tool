...
<div class="modal-body bg-additional-grey">
    <x-cards.data>
        ...
        <x-cards.data-row :label="__('modules.projects.milestoneStartDate')" :value="is_null($milestone->start_date) ? '--':$milestone->start_date->format(company()->date_format)" />

        <x-cards.data-row :label="__('modules.projects.milestoneEndDate')" :value="is_null($milestone->end_date) ? '--':$milestone->end_date->format(company()->date_format)" />
        ...
        <x-table class="border-0 pb-3 admin-dash-table table-hover">
            ...
            @endphp
                <tr>
                    ...
                    <td>{{ $item->due_date ? $item->due_date->format(company()->date_format) : '--' }}</td>
                    <td>{{$totalTimeLog}}</td>
                    <td>
