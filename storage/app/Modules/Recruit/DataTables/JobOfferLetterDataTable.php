...
class JobOfferLetterDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('job', function ($row) {
                return '<a href="' . route('jobs.show', [$row->job_id]) . '" class="openRightModal text-darkest-grey" >' . ucfirst($row->title) . '</a>';
            })

            ->editColumn('job_applicant', function ($row) {
                return '<a href="' . route('job-applications.show', [$row->job_app_id]) . '" class=" text-darkest-grey openRightModal" >' . ucfirst($row->full_name) . '</a>';
                ;
            })

            ->editColumn('job_expire_on', function ($row) {
                return $row->job_expire;
            })
            ->editColumn('exp_joining_date', function ($row) {
                return $row->expected_joining_date;
            })
            ...
            ->addColumn('action', function ($row) {
                $date1 = Carbon::createFromFormat('Y-m-d', $row->job_expire);
            ...
            ->addIndexColumn()
            ->rawColumns(['action', 'job','job_applicant', 'job_expire_on','exp_joining_date','check','status'])
    ...
    public function query(RecruitJobOfferLetter $jobs)
    {
        ...
        $jobs = $jobs->select('recruit_job_offer_letter.employee_id', 'recruit_job_offer_letter.status', 'recruit_job_offer_letter.id', 'recruit_job_offer_letter.job_id', 'recruit_job_offer_letter.job_app_id', 'recruit_job_offer_letter.job_expire', 'recruit_job_offer_letter.expected_joining_date', 'recruit_job_applications.full_name', 'recruit_jobs.title', 'recruit_jobs.recruiter_id');

        $jobs = $jobs->leftJoin('recruit_jobs', 'recruit_jobs.id', '=', 'recruit_job_offer_letter.job_id')
            ->leftJoin('recruit_job_applications', 'recruit_job_applications.id', '=', 'recruit_job_offer_letter.job_app_id')
    ...
    protected function getColumns()
    {
        return [
            ...
            __('recruit::modules.joboffer.job') => ['data' => 'job', 'name' => 'job'],
            __('recruit::app.jobOffer.jobApplicant') => ['data' => 'job_applicant', 'name' => 'job_applicant'],
            __('recruit::modules.joboffer.OfferExp') => ['data' => 'job_expire_on', 'name' => 'job_expire_on'],
            __('recruit::app.jobOffer.expJoinDate') => ['data' => 'exp_joining_date', 'name' => 'exp_joining_date'],
            __('recruit::app.jobOffer.jobApplicant') => ['data' => 'job_applicant', 'name' => 'job_applicant'],
            __('recruit::modules.joboffer.status') => ['data' => 'status', 'name' => 'status'],
