...
class ApplicantNoteController extends AccountBaseController
{
    ...
    public function store(StoreJobApplicant $request)
    {
        ...
        $note->note_text = $request->comment;
        $note->user_id = $this->user->id;
        $note->job_application_id = $request->applicationId;
        $note->save();

        $this->comments = RecruitApplicantNote::with('user')->where('job_application_id', $request->applicationId)->orderBy('id', 'desc')->get();
    ...
    public function destroy($id)
    {
        ...
        $comment_task_id = $notes->job_application_id;
        $notes->delete();

        $this->comments = RecruitApplicantNote::with('user')->where('job_application_id', $comment_task_id)->orderBy('id', 'desc')->get();
        $view = view('recruit::job-applications.notes.show', $this->data)->render();
