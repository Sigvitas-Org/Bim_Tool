            ...
            <div class="add-client bg-white rounded">
                ...
                <div class="row p-20">
                    ...
                    <div class="col-md-3 status">
                        <x-forms.label class="mt-3" fieldId="status_id" :fieldLabel="__('recruit::modules.jobApplication.status')"
                        fieldRequired="true">
                        </x-forms.label>
                        <x-forms.input-group>
                            <x-forms.input-group>
                                <select class="form-control select-picker" name="status_id"
                                    ...
                                    @foreach ($statuses as $status)
                                        <option @if ($evaluation->status_id == $status->id) selected @endif value="{{ $status->id }}">{{ $status->status }}</option>
                                    @endforeach
                                </select>
                    ...
                    <div class="col-md-4">
                        <x-forms.textarea class="mr-0 mr-lg-2 mr-md-2"
                            :fieldLabel="__('recruit::app.menu.details')" fieldName="details" fieldRequired="true"
                            fieldId="details" :fieldValue="$evaluation->details" :fieldPlaceholder="__('recruit::app.menu.details')">
                        </x-forms.textarea>
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('body').on('click', '#save-evaluation', function() {
