...
<div id="task-detail-section">
    <h3 class="heading-h1 mb-3">{{ ucfirst($interview->jobApplication->full_name) }}</h3>
    <div class="d-flex filter-box project-header bg-white">
        ...
        <div class="project-menu d-lg-flex" id="mob-client-detail">
            ...
            @if ($interview->parent_id == null)
                <x-tab :href="route('interview-schedule.show', $interview->id)" :text="$interview->stage ? $interview->stage->name : ''"
                    class="active"/>
            @else
                <x-tab :href="route('interview-schedule.show', $interview->parent_id)" :text="$parentStage->stage ? $parentStage->stage->name : ''"
                    class="stage"/>
            @endif
            @foreach($childInterviews as $childInterview)
                @if ($interview->id == $childInterview->id)
                <a href="{{ route('interview-schedule.show', $childInterview->id) }}" class = 'text-dark-grey text-capitalize border-right-grey p-sub-menu active'><span>{{ $childInterview->stage->name }}</span></a>
                @else
                <a href="{{ route('interview-schedule.show', $childInterview->id) }}" class = 'text-dark-grey text-capitalize border-right-grey p-sub-menu'><span>{{ $childInterview->stage->name }}</span></a>
                @endif
            @endforeach
                                ...
                                @foreach ($selected_employees as $attendee)
                                    @if (!in_array($attendee, $submitted) && $attendee == user()->id)
                                        <div class="d-flex flex-wrap">
                                            <x-forms.link-primary :link="route('evaluation.create', ['id' => $interview_schedule_id])" class="mr-3 openRightModal" icon="plus">
                                ...
                                @if ($interview->video_type == 'zoom')
                                    @php
                                        if ($zoom_setting->meeting_app == 'in_app') {
                                            ...
                                            $url = user()->id == $interview->meeting->created_by ? $interview->meeting->start_link : $interview->meeting->end_link;
                                        }
                                        $nowDate = now(global_setting()->timezone)->toDateString();
                                    @endphp
                                    <div class="col-md-3">
                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0"
                                    ...
                                    @if (in_array($loggedEmployee->id, $secEmp) && $employeeStatus->user_accept_status == 'waiting' && $interview->status == 'pending')
                                        <a class="dropdown-item employeeResponse" data-response-id={{ $employeeStatus->id }}
                                        data-response-action="accept" href="javascript:;">@lang('app.accept')
                                        </a>
                                        <a class="dropdown-item employeeResponse" data-response-id={{ $employeeStatus->id }}
                                        data-response-action="accept" href="javascript:;">@lang('app.reject')
                                        </a>
                                    @endif
                ...
                <div class="card-body">
                    ...
                    <div class="col-12 px-0 pb-3 d-lg-flex d-md-flex d-block">
                        ...
                        <p class="mb-0 text-dark-grey f-14 w-70 text-wrap p-0">
                            <a href="{{ route('jobs.show', [$interview->jobApplication->job->id])}}" class="text-dark-grey openRightModal">{{ ucfirst($interview->jobApplication->job->title) }}</a>
                        </p>
                    </div>
                    ...
                    <div class="col-12 px-0 pb-3 d-lg-flex d-md-flex d-block">
                        ...
                        <p class="mb-0 text-dark-grey f-14 w-70 text-wrap p-0">
                            <a href="{{ route('job-applications.show', [$interview->jobApplication->id])}}" class="text-dark-grey openRightModal">{{ ucfirst($interview->jobApplication->full_name) }}</a>
                        </p>
                    </div>

                    <x-cards.data-row :label="__('recruit::modules.interviewSchedule.candidateEmail')"
                        :value="ucwords($interview->jobApplication->email) ?? '--'" />
                    <x-cards.data-row :label="__('app.phone')"
                        :value="ucwords($interview->jobApplication->phone) ?? '--'" />
                    ...
                    :value="ucwords($interview->phone) ?? '--'" />
                    @endif

                    <x-cards.data-row :label="__('recruit::modules.interviewSchedule.startOn')"
                    :value="$interview->schedule_date->format($global->date_format. ' - ' . $global->time_format)" />
                    ...
                    <div class="col-12 px-0 pb-3 d-lg-flex d-md-flex d-block">
                        ...
                        <p class="mb-0 text-dark-grey f-14 w-70 text-wrap p-0">
                            @if ($interview->status == 'pending')
                                <i class="fa fa-circle mr-1 text-yellow f-14"></i> {{ ucfirst($interview->status) }}
                            @elseif ($interview->status == 'completed')
                                <i class="fa fa-circle mr-1 text-blue f-14"></i>{{ ucfirst($interview->status) }}
                            @elseif ($interview->status == 'hired')
                                <i class="fa fa-circle mr-1 text-light-green f-14"></i>{{ ucfirst($interview->status) }}
                            @elseif ($interview->status == 'rejected')
                                <i class="fa fa-circle mr-1 text-brown f-14"></i>{{ ucfirst($interview->status) }}
                            @else
                                <i class="fa fa-circle mr-1 text-red f-14"></i>{{ ucfirst($interview->status) }}
                            @endif
                        </p>
                    </div>
                                    ...
                                    <div class="col-10">
                                        <p class="mb-0 text-dark-grey f-14">
                                            @if ($item->user_accept_status == 'waiting')
                                                <x-status :value="__('recruit::modules.interviewSchedule.awaiting')" color="yellow" />
                                            @elseif ($item->user_accept_status == 'refuse')
                                                <x-status :value="__('recruit::modules.interviewSchedule.refused')" color="red" />
                                            @elseif ($item->user_accept_status == 'accept')
                                                <x-status :value="__('recruit::modules.interviewSchedule.accepted')" color="dark-green" />
                                            @endif
                                        </p>
                                    </div>
                                </div>
                    ...
                    @if ($interview->interview_type == 'video')
                        @if ($interview->video_type == 'other')
                            <x-cards.data-row :label="__('recruit::modules.interviewSchedule.link')"
                                :value="ucwords($interview->other_link) ?? '--'" />
                        @endif
                        ...
                        @if (in_array('Zoom', $worksuitePlugins))
                            ...
                            @if ($interview->video_type == 'zoom')
                                ...
                                <x-cards.data-row :label="__('recruit::modules.interviewSchedule.startOn')"
                                    :value="$interview->meeting->start_date_time->format($global->date_format. ' - ' . $global->time_format)" />

                                <x-cards.data-row :label="__('zoom::modules.zoommeeting.endOn')"
                                    :value="$interview->meeting->end_date_time->format($global->date_format. ' - ' . $global->time_format)" />
                                ...
                                <div class="col-12 px-0 pb-3 d-flex">
                                    ...
                                    <div class="taskEmployeeImg rounded-circle mr-1">
                                        <img data-toggle="tooltip"
                                            data-original-title="{{ ucwords($interview->meeting->host->name) }}"
