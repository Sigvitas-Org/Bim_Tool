        ...
        <div class="add-client bg-white rounded">
            <div id="stage-select-update">
                <x-form id="save-event-data-form">
                    ...
                    <div class="row py-20">
                        <input type="hidden" name="stage_id" value="{{ $interview->stage_id ? $interview->stage_id : '' }}">

                        <div class="col-md-3">
                            <x-forms.label fieldId="candidate_id"
                                :fieldLabel="__('recruit::modules.interviewSchedule.candidate')"
                                class="mt-3"></x-forms.label>
                            <input type="hidden" name="candidate_id" value="{{$interview->job_application_id}}">
                            <select disabled name="candidate_id" id="candidate_id" class="form-control select-picker" data-size="8">
                                <option value="">--</option>
                                    @foreach ($candidates as $candidate)
                                        <option @if($interview->job_application_id == $candidate->id) selected @endif value="{{ $candidate->id }}">{{ ucwords($candidate->full_name) }}</option>
                                    @endforeach
                            </select>
                        </div>

                        <div class="col-md-3">
                            <x-forms.select fieldId="selectEmployee" :fieldLabel="__('recruit::modules.interviewSchedule.interviewer')"
                                fieldName="employee_id[]" fieldRequired="true" search="true" multiple="true">
                            @foreach ($employees as $emp)
                                    <option
                                    @if(in_array($emp->id, $selected_employees)) selected @endif
                        ...
                        <div class="col-md-3">
                            <x-forms.select fieldId="interview_type" :fieldLabel="__('recruit::modules.interviewSchedule.interviewType')"
                                fieldName="interview_type" >
                                <option value="in person" @if($interview->interview_type == 'in person') selected @endif>@lang('recruit::app.interviewSchedule.inPerson')</option>
                                <option value="video" @if($interview->interview_type == 'video') selected @endif>@lang('recruit::app.interviewSchedule.video')</option>
                                <option value="phone" @if($interview->interview_type == 'phone') selected @endif>@lang('recruit::modules.jobApplication.phone')</option>
                            </x-forms.select>
                        </div>

                        <div class="col-md-3 py-20">
                            <x-forms.select fieldId="status" :fieldLabel="__('recruit::modules.jobApplication.status')" fieldName="status"
                                search="true">
                                <option value=""> -- </option>
                                <option  @if($interview->status == 'rejected') selected @endif value="rejected" data-content="<i class='fa fa-circle mr-2 text-brown'></i> @lang('recruit::app.interviewSchedule.rejected')"></option>
                                <option  @if($interview->status == 'hired') selected @endif value="hired" data-content="<i class='fa fa-circle mr-2 text-light-green'></i> @lang('recruit::app.interviewSchedule.hired')"></option>
                                <option  @if($interview->status == 'pending') selected @endif value="pending" data-content="<i class='fa fa-circle mr-2 text-yellow'></i> @lang('recruit::app.interviewSchedule.pending')"></option>
                                <option  @if($interview->status == 'completed') selected @endif value="completed" data-content="<i class='fa fa-circle mr-2 text-blue'></i> @lang('app.completed')"></option>
                                <option  @if($interview->status == 'canceled') selected @endif value="canceled" data-content="<i class='fa fa-circle mr-2 text-red'></i> @lang('recruit::app.interviewSchedule.canceled')"></option>
                            </x-forms.select>
                        </div>
                        ...
                        <div class="col-md-3 d-none" id="phone">
                            <x-forms.text fieldId="phone" :fieldLabel="__('recruit::modules.jobApplication.phone')" fieldName="phone"
                            :fieldValue="($interview)?$interview->phone : ''" fieldRequired="true" />
                        </div>
                        ...
                        <div class="col-md-4 d-none" id=type>
                            <div class="form-group my-3">
                                <x-forms.label fieldId=""
                                    :fieldLabel="__('recruit::modules.interviewSchedule.interviewType')"></x-forms.label>
                                <div class="d-flex">
                                    ...
                                    <x-forms.radio fieldId="video_type" :fieldLabel="__('recruit::modules.interviewSchedule.other')"
                                        fieldName="video_type" fieldValue="other" :checked="($interview->video_type == 'other')">
                                    </x-forms.radio>
                        ...
                        <div class="row py-20 display" id="repeat-fields">
                            <div class="col-md-6">
                                ...
                                fieldPlaceholder="" :fieldValue="($interview->meeting)?$interview->meeting->meeting_name : ''" />
                            </div>
                            ...
                            <div class="col-md-4">
                                <x-forms.select fieldId="created_by" :fieldLabel="__('recruit::modules.interviewSchedule.meetingHost')" fieldName="created_by" search="true">
                                    @if ($interview->meeting)
                                        @foreach ($employees as $emp)
                                            <option @if ($emp->id == $interview->meeting->created_by) selected @endif
                                                data-content="<div class='d-inline-block mr-1'><img class='taskEmployeeImg rounded-circle' src='{{ $emp->image_url }}' ></div> {{ ucfirst($emp->name) }}"
                                                value="{{ $emp->id }}">{{ ucwords($emp->name) }}</option>
                                        @endforeach
                                    @else
                            ...
                            <div class="col-md-4">
                                <div class="form-group my-3">
                                    <x-forms.label fieldId="host_video"
                                        :fieldLabel="__('recruit::modules.interviewSchedule.hostVideoStatus')"></x-forms.label>
                                    <div class="d-flex">
                                        <x-forms.radio fieldId="host_video1" :fieldLabel="__('app.enable')"
                                            fieldName="host_video" fieldValue="1" :checked="
                                            ($interview->meeting)?$interview->meeting->host_video == 1 : ''">
                                        </x-forms.radio>

                                        <x-forms.radio fieldId="host_video2" :fieldLabel="__('app.disable')"
                                            fieldName="host_video" fieldValue="0" :checked="($interview->meeting)?$interview->meeting->host_video == 0 : ''" checked>
                                        </x-forms.radio>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-4">
                                <div class="form-group my-3">
                                    <x-forms.label fieldId="participant_video"
                                        :fieldLabel="__('recruit::modules.interviewSchedule.participantVideoStatus')"></x-forms.label>
                                    <div class="d-flex">
                                        <x-forms.radio fieldId="participant_video1" :fieldLabel="__('app.enable')"
                                            fieldName="participant_video" fieldValue="1"
                                            :checked="($interview->meeting)?$interview->meeting->participant_video == 1 : ''">
                                        </x-forms.radio>
                                        <x-forms.radio fieldId="participant_video2" :fieldLabel="__('app.disable')"
                                            fieldValue="0" fieldName="participant_video"
                                            :checked="($interview->meeting)?$interview->meeting->participant_video == 0 : ''" checked>
                                        </x-forms.radio>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-lg-4">
                                        <x-forms.number class="mr-0 mr-lg-2 mr-md-2"
                                            :fieldLabel="__('recruit::modules.interviewSchedule.remindBefore')" fieldName="remind_time"
                    ...
                    <div class="col-md-6 py-20 d-none" id="video">
                        <x-forms.text fieldId="other_link" :fieldLabel="__('recruit::modules.interviewSchedule.otherLink')" fieldName="other_link" :fieldValue="($interview)?$interview->other_link : ''" fieldRequired="true" />
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('body').on('click', '#save-event-form', function() {
        ...
        @elseif($interview->interview_type == 'video')
            ...
            @else
                ...
                $('#type').removeClass('d-none');
            @endif
        @else($interview->interview_type == 'phone')
