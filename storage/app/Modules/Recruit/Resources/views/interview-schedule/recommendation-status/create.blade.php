...
<div class="modal-body">
    <x-table class="table-bordered" headType="thead-light">
        ...
        @forelse($statuses as $key=>$item)
            <tr id="row-{{ $item->id }}">
                <td>{{ $key + 1 }}</td>
                <td data-row-id="{{ $item->id }}" contenteditable="true">{{ ucwords($item->status) }}</td>
                <td class="text-right">
    ...
    <x-form id="createRecommStatus">
        <div class="row border-top-grey ">
            <div class="col-sm-12">
                <x-forms.text fieldId="status" :fieldLabel="__('recruit::modules.jobApplication.status')" fieldName="status"
                    fieldRequired="true" :fieldPlaceholder="__('recruit::app.interviewSchedule.statusPlaceholder')">
                </x-forms.text>
            </div>
        </div>
    </x-form>
...
<script>
    ...
    $('body').on('click', '#save-status', function() {
