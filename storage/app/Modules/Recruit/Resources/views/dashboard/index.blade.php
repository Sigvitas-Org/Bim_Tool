        ...
        <div class="row row-cols-lg-4 my-3">
            ...
            <div class="col mb-4">
                <a href="{{ route('jobs.index') }}" data-status="closed" class="widget-filter-status">
                    <x-cards.widget :title="__('recruit::app.dashboard.totalOpenings')"
                        value="{{ $totalOpenings }}" icon="ticket-alt" widgetId="closedTickets" />
                </a>
            </div>
            ...
            <div class="col mb-4">
                <a href="{{ route('job-appboard.index') }}" data-status="pending" class="widget-filter-status">
                    <x-cards.widget :title="__('recruit::app.dashboard.totalHired')" value="{{ $totalHired }}" icon="ticket-alt"
            ...
            <div class="col">
                <a href="{{ route('job-appboard.index') }}" data-status="resolved" class="widget-filter-status">
                    <x-cards.widget :title="__('recruit::app.dashboard.totalRejected')" value="{{ $totalRejected }}" icon="ticket-alt"
                        widgetId="resolvedTickets" />
                </a>
            </div>
            <div class="col">
                <a href="{{ route('job-appboard.index') }}" data-status="resolved" class="widget-filter-status">
                    <x-cards.widget :title="__('recruit::app.dashboard.newApplications')" value="{{ $newApplications }}" icon="ticket-alt"
                        widgetId="resolvedTickets" />
                </a>
            </div>
            <div class="col">
                <a href="{{ route('job-appboard.index') }}" data-status="resolved" class="widget-filter-status">
                    <x-cards.widget :title="__('recruit::app.dashboard.shortlistedCandidate')" value="{{ $shortlisted }}" icon="ticket-alt"
                        widgetId="resolvedTickets" />
                </a>
            </div>
            <div class="col">
                <a href="{{ route('interview-schedule.index') }}" data-status="resolved" class="widget-filter-status">
                    <x-cards.widget :title="__('recruit::app.dashboard.todayInterview')" value="{{ $totalTodayInterview }}" icon="ticket-alt"
                        widgetId="resolvedTickets" />
                </a>
            </div>
            ...
            <div class="col-sm-12 col-lg-6 mt-3">
                <x-cards.data :title="__('recruit::app.dashboard.source')">
                    <x-pie-chart id="task-chart" :labels="$applicationSourceWise['labels']"
                        :values="$applicationSourceWise['values']" :colors="$applicationSourceWise['colors']" height="250" width="300" />
                </x-cards.data>
            </div>
            <div class="col-sm-12 col-lg-6 mt-3">
                <x-cards.data :title="__('recruit::app.dashboard.applicationStatus')">
                    <x-pie-chart id="task-chart-1" :labels="$candidateStatusWise['labels']"
                        :values="$candidateStatusWise['values']" :colors="$candidateStatusWise['colors']" height="250" width="300" />
                </x-cards.data>
            </div>
                        ...
                        @forelse ($activeJobs as $item)
                            <tr>
                                <td class="pl-20">
                                    ...
                                    <a href="{{ route('jobs.show', [$item->id]) }}"> {{ ucfirst($item->title) }} </a>
                                    </h5>
                                </td>
                                <td class="text-darkest-grey">{{ $item->end_date ? $item->end_date->format(global_setting()->date_format) : __('recruit::modules.job.noEndDate') }}</td>
                                <td>
                                    ...
                                    <td class="pl-20">
                                        <h5 class="f-13 text-darkest-grey">
                                            ...
                                            || ($viewInterviewPermission == 'both' && (in_array($loggedEmployee->id, $secEmp)) || $item->added_by == user()->id)))
                                                ...
                                                class="openRightModal">{{ $item->jobApplication ? ucfirst($item->jobApplication->full_name) : ''}}</a>
                                            @else
                                                <a href="javascript:;">{{ $item->jobApplication ? ucfirst($item->jobApplication->full_name) : '' }}</a>
                                            @endif
                                        </h5>
                                            <div class="text-muted">{{ $item->jobApplication ? ucfirst($item->jobApplication->job->title) : '' }}</div>
                                    </td>
                                    <td class="text-darkest-grey">{{ $item->schedule_date->format(global_setting()->date_format) }}</td>
                                    <td>
                                        @php
                                        ...
                                        @endphp
                                        <x-status :color="$priority" :value="__('recruit::app.interviewSchedule.' . $item->status)" />
                                    </td>
