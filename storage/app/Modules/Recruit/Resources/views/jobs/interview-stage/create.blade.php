    ...
    <x-form id="createStages">
        <div class="row border-top-grey ">
            <div class="col-sm-12">
                ...
                <x-forms.text fieldId="name" :fieldLabel="__('recruit::app.interviewSchedule.stages')" fieldName="name" fieldRequired="true"
...
<script>
    ...
    $('body').on('click', '.delete-row', function() {
        ...
        var url = "{{ route('interview-stages.destroy', ':id') }}";
    ...
    $('body').on('click', '#save-stage', function() {
        var url = "{{ route('interview-stages.store') }}";
        $.easyAjax({
            url: url,
    ...
    $('#stage-table [contenteditable=true]').focus(function() {
        ...
        if ($(this).data("initialText") !== $(this).html()) {
            ...
            let value = $(this).html();
            var url = "{{ route('interview-stages.update', ':id') }}";
            url = url.replace(':id', id);
            var token = "{{ csrf_token() }}";
            var selectedStages = "{{ $selectedStages }}";
            $.easyAjax({
                url: url,
                container: '#row-' + id,
                type: "PUT",
