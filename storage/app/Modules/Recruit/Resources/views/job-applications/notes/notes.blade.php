                    ...
                    <div class="media">
                        <div class="media-body bg-white">
                            <div class="form-group">
                                <x-forms.label class="" fieldId="application-note"
                                :fieldLabel="__('app.add'). ' ' . __('app.note')">
                                </x-forms.label>
                                <div id="application-note"></div>
                                <textarea name="application-note" class="form-control invisible d-none" id="application-note-text"></textarea>
                            </div>
                        </div>
                    </div>
            ...
            <div class="card w-100 rounded-0 border-0 comment">
                <div class="card-horizontal">
                    <div class="card-body border-0 pl-0 py-1">
                        ...
                        <div class="card-text f-14 text-dark-grey text-justify ql-editor">{!! ucfirst($comment->note_text) !!}</div>
                    </div>
                </div>
            </div>
        @empty
...
<script>
    ...
    quillImageLoad('#application-note');
    $(document).ready(function() {

        $('body').on('click', '#submit-note', function() {

            var comment = document.getElementById('application-note').children[0].innerHTML;
            document.getElementById('application-note-text').value = comment;
            ...
            $.easyAjax({
                ...
                success: function(response) {
                    if (response.status == "success") {
                        ...
                        document.getElementById('application-note').children[0].innerHTML = "";
