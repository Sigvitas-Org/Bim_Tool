...
class OfferLetterObserver
{
    ...
    {
        if (\user()) {
            $this->logOfferLetterActivity($event->job_id, $event->id, user()->id, 'createLetter');
        }
    }
    ...
    public function updated(RecruitJobOfferLetter $event)
    {
        if (\user()) {
            if ($event->status == 'accept') {
                $this->logOfferLetterActivity($event->job_id, $event->id, user()->id, 'acceptLetter');
            }
            elseif ($event->status == 'decline') {
                $this->logOfferLetterActivity($event->job_id, $event->id, user()->id, 'declineLetter');
            }
            else {
                $this->logOfferLetterActivity($event->job_id, $event->id, user()->id, 'updateLetter');
            }
        }
