...
class JobApplicationsObserver
{
    ...
    public function created(RecruitJobApplication $event)
    {
        if (!isRunningInConsoleOrSeeding()) {
            if (\user()) {
                $this->logRecruitJobsActivity($event->job_id, user()->id, 'createJobapplication', $event->id, null);
            }
    ...
    public function updating(RecruitJobApplication $event)
    {
        if (!isRunningInConsoleOrSeeding()) {
            if (\user()) {
                $this->logRecruitJobsActivity($event->job_id, user()->id, 'updateJobapplication', $event->id, null);
            }
        }
    }
