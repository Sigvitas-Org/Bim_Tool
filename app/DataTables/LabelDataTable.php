    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('label_name', function ($row) {
                ...
                if ($row->color) {
                    return '<label class="badge" style="background: ' . $row->color . ';">' . mb_ucwords($row->label_name) . '</label>';
                }

                return '<label class="badge"  style="background:#3b0ae1;">' . mb_ucwords($row->label_name) . '</label>';
            })
            ->editColumn('description', function ($row) {
                if ($row->description) {
                    return mb_ucwords($row->description);
                }
