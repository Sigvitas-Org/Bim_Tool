...
class ProjectsDataTable extends BaseDataTable
{
    ...
    public function __construct()
    {
        ...
        $this->changeStatusPermission = user()->permission('change_project_status');
    }
    ...
    public function dataTable($query)
    {
        ...
        $datatables->editColumn('deadline', function ($row) {
            if ($row->deadline) {
                return $row->deadline->format($this->company->date_format);
            }
        ...
        $datatables->addColumn('status', function ($row) use ($projectStatus) {
            ...
            if (
                ...
                foreach ($projectStatus as $item) {
                    ...
                    $status .= '  data-content="<i class=\'fa fa-circle mr-2\' style=\'color: ' . $item->color . '\'></i> ' . ucwords($item->status_name) . '" value="' . $item->status_name . '" >' . ucwords($item->status_name) . '</option>';
                }
                ...
                foreach ($projectStatus as $item) {
                    if ($row->status == $item->status_name) {
                        return '<i class="fa fa-circle mr-1 text-yellow"
                            style="color: ' . $item->color . '"></i>' . ucwords($item->status_name);
                    }
                }
            }
