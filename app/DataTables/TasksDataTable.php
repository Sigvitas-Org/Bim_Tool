...
class TasksDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->editColumn('due_date', function ($row) {
            ...
            if ($row->due_date->endOfDay()->isPast()) {
                return '<span class="text-danger">' . $row->due_date->format($this->company->date_format) . '</span>';
            }
            elseif ($row->due_date->isToday()) {
                return '<span class="text-success">' . __('app.today') . '</span>';
            }

            return '<span >' . $row->due_date->format($this->company->date_format) . '</span>';
        });
        ...
        $datatables->addColumn('short_code', function ($row)  {
            ...
            return BaseModel::clickAbleLink(route('tasks.show', [$row->id]), $row->task_short_code);
        });
        ...
        $datatables->editColumn('heading', function ($row) {
            ...
            if ($row->incomplete_subtasks_count > 0) {
                $subTask .= '<a href="' . route('tasks.show', [$row->id]) . '?view=sub_task" class="openRightModal"><span class="border rounded p-1 f-11 mr-1 text-dark-grey" data-toggle="tooltip" data-original-title="' . __('modules.tasks.subTask') . '"><i class="bi bi-diagram-2"></i> ' . $row->incomplete_subtasks_count . '</span></a>';
            }
    ...
    public function query(Task $model)
    {
        ...
        $model->leftJoin('users as creator_user', 'creator_user.id', '=', 'tasks.created_by')
            ...
            ->withCount('activeTimerAll', 'incompleteSubtasks')
            ->groupBy('tasks.id');
