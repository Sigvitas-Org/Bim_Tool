...
class TaskNoteClient extends BaseNotification
{
    ...
    public function toMail($notifiable)
    {
        ...
        return parent::build()
            ...
            ->line(__('email.taskNote.subject') . ' - ' . ucfirst($this->task->heading) . ' #' . $this->task->task_short_code)
            ->line((!is_null($this->task->project)) ? __('app.project') . ' - ' . ucfirst($this->task->project->project_name) : '')
            ->action(__('email.taskNote.action'), $url)
...
//phpcs:ignore
    ...
    public function toOneSignal($notifiable)
    {
        return OneSignalMessage::create()
            ...
            ->body(__('email.taskNote.subject') . ' - ' . ucfirst($this->task->heading) . ' #' . $this->task->task_short_code);
    }
