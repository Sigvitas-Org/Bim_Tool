...
class EmployeeController extends AccountBaseController
{
    ...
    public function store(StoreRequest $request)
    {
        ...
        if (request()->add_more == 'true') {
            ...
            return Reply::successWithData(__('messages.employeeAdded'), ['html' => $html, 'add_more' => true]);
        }
    ...
    public function importStore(ImportRequest $request)
    {
        ...
        $excelData = Excel::toArray(new EmployeeImport, public_path(Files::UPLOAD_FOLDER . '/' . Files::IMPORT_FOLDER . '/' . $this->file))[0];
        $this->hasHeading = $request->has('heading');
        $this->heading = array();
        $this->fileHeading = array();

        $this->columns = EmployeeImport::fields();
    ...
    public function importProcess(ImportProcessRequest $request)
    {
        ...
        $excelData = Excel::toArray(new EmployeeImport, public_path(Files::UPLOAD_FOLDER . '/' . Files::IMPORT_FOLDER . '/' . $request->file))[0];
