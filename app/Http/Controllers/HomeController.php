...
class HomeController extends Controller
{
    ...
    public function invoice($hash)
    {
        ...
        $this->invoice = Invoice::with('currency', 'project', 'project.client', 'items.invoiceItemImage', 'items')->where('hash', $hash)->firstOrFail();
    ...
    {
        $this->task = Task::with('boardColumn', 'project', 'users', 'label', 'approvedTimeLogs', 'approvedTimeLogs.user', 'comments', 'comments.user')->withCount('subtasks', 'files', 'comments', 'activeTimerAll')
        ...
        case 'history':
            ...
            break;
        case 'comments':
            $this->tab = 'front.tasks.ajax.comments';
            break;
        case 'time_logs':
            $this->tab = 'front.tasks.ajax.timelogs';
            break;
        case 'notes':
            $this->tab = 'front.tasks.ajax.notes';
            break;
        default:
    ...
    public function ticketStore(StoreCustomTicket $request)
    {
        ...
        if (!$existing_user) {
            ...
            // attach role
            $role = Role::where('name', 'client')->where('company_id', $company->id)->select('id')->first();
